...
## 2026-09-15 — Integration log insert runs in system mode (same failure class as DEF-001)

- Decision: `MGAgenciaIntegrationLogger.log()` now inserts `MGAgencia_Integration_Log__c` in SYSTEM_MODE via a private `without sharing` inner class (`SystemModeLogWriter`), mirroring `MGAgenciaDuplicateEvaluator`'s `SystemModeDuplicateScanner`. Every other operation in the class stays untouched.
- Reason: found while investigating why zero integration log records existed for real production Leads created by the actual least-privilege integration user (`mgagencia.integration@condor.com.py`), despite every admin-run test passing. Root cause: since Decision 1's 2026-09-08 amendment, created Leads end up owned by queue MQL, which the integration user is not a member of. The log record's `Lead__c` lookup references that Lead, so the sharing-enforced `USER_MODE` insert failed silently (caught, only `System.debug`'d) — identical failure class to DEF-001, just never applied to the logger. This broke the project's core traceability requirement (CLAUDE.md §"Every external request must have traceability") for every real request since that amendment went live.
- Alternatives: adding the integration user to queue MQL (rejected — same over-exposure reasoning as DEF-001, grants broad record visibility to a non-human user); logging before the Lead insert so `Lead__c` is never populated on the log's own insert (rejected — would only mask the symptom for the CREATED path and still fail for any future field on the log object that looks up a sharing-restricted record).
- Impact: `MGAgenciaIntegrationLogger` refactor + regression test `integrationLogIsWrittenUnderLeastPrivilegeIntegrationUser` (`MGAgenciaLeadServiceTest`), confirmed to reproduce the failure pre-fix (0 log rows under the restricted test user) and pass post-fix. **Known gap, not addressed here**: real production Leads created before this fix has no corresponding log record — backfilling those (if desired/possible) is a separate, unimplemented decision.

## 2026-09-15 — Dynamic LeadSource per platform + Meta identifier fields (PROVISIONAL)

- Decision: add 5 new optional request fields — `platform`, `meta_lead_id`, `meta_form_id`, `campaign_name`, `campaign_description` — per an admin/marketing request ("Cambios MG Agencia.docx"). `platform` (`facebook`/`instagram`/`tiktok`/`web`) resolves `LeadSource` via a new `MGAgenciaPlatformResolver` (`facebook`/`instagram` → `Redes Sociales Empresa`, `tiktok` → `Tiktok`, `web` → `Web`); the other 4 are direct pass-through to `ID_Meta__c`, `Identificador_Lead_Capture__c`, `campa_a_meta__c`, `Descripci_n_de_Meta__c`.
- Reason: the business wants MGAgencia Leads to route/report through the same origin-based channels used for organic Meta leads, and to carry the same Meta campaign/identifier data those leads already carry. `interest_model` not reaching Salesforce was raised in the same request; investigated separately — evidence (100% of the 10 most recent real MGAgencia Leads have `VehicleOfInterestOfTheWeb__c` null, while our existing mapping already works when the value is sent) points to MGAgencia simply not sending it, not a mapping defect on our side. No code change made for that; relayed to MGAgencia to confirm.
- Alternatives considered: (a) keep the fixed `LeadSource = 'MGAgencia'` and have a separate internal process re-map by platform after creation — rejected, adds a second moving part for no benefit when the source system can just tell us; (b) require `platform` — rejected, would break current production traffic (10 real Leads/day today, all without this field) the moment this deploys.
- Backward compatibility: all 5 fields are optional and additive. `platform` absent/blank keeps the current fixed `LeadSource` from `MGAgencia_Integration_Setting__mdt.Lead_Source__c`, unchanged. `platform` present but not one of the 4 known values is rejected with 422 `UNKNOWN_VALUE`, never silently defaulted.
- Discovery evidence: `Redes Sociales Empresa`/`Web` are active LeadSource values in both `condor-qas` and `mi-org`; `Tiktok` (exact casing) is active only in `mi-org` — condor-qas is stale on this value (same drift class as Decision 7's branch-casing bug, docs/FIELD_MAPPING.md §13). The 4 new Lead fields are confirmed to exist (free text, no dependent picklist) but nothing in the org's Flows/Triggers/Apex references `ID_Meta__c` or `Identificador_Lead_Capture__c` — their mapping to MGAgencia's raw data is our own contract design, not a reverse-engineered internal rule. The existing active flow `Actualizar_Campa_as_Digitales` already auto-creates/links a Campaign from `campa_a_meta__c` once `LeadSource` resolves into a "Redes Sociales" value — no new Salesforce-side automation needed.
- Status: **PROVISIONAL — implemented in `condor-qas` only, not deployed to `mi-org`.** MGAgencia has not yet confirmed they can send these fields; a heads-up was sent to them. Final field names/values may change based on their response. See the 2026-09-16 follow-up below — MGAgencia confirmed most of this and one field name changed as a result.
- Impact: `MGAgenciaLeadRequest`, `MGAgenciaLeadValidator`, `MGAgenciaLeadService.buildLead()`, new `MGAgenciaPlatformResolver`; `docs/API_SPEC.md` §3 and `docs/FIELD_MAPPING.md` §13 updated. Full test suite still passing (see PR).

## 2026-09-16 — MGAgencia confirmation: 3 fields confirmed, `campaign_description` never available, `meta_lead_id` dropped

- MGAgencia replied to the 2026-09-15 contract preview with concrete answers, superseding parts of that entry:
  - `platform`, `meta_form_id`, `campaign_name` — confirmed, can be sent as designed.
  - `campaign_description` — **permanently unavailable**; the ad platform does not provide this data to MGAgencia. The field/mapping (`Descripci_n_de_Meta__c`) stays in the contract for forward compatibility (e.g. a future non-Meta origin), but will realistically always be null for Meta-sourced leads. No code change needed — it was already optional.
  - `meta_lead_id` — **dropped**. MGAgencia confirmed Meta's individual lead id already arrives via the existing `external_lead_id` field (same data, already part of the contract since the original v1 design, already mapped to `External_Lead_Id__c` and used for duplicate detection). The separate `meta_lead_id` request field and its validation were removed; `ID_Meta__c` is now populated by copying the same trimmed `external_lead_id` value already computed for `External_Lead_Id__c` in `MGAgenciaLeadService.buildLead()` — no second field, no new normalization.
  - `interest_model` — MGAgencia fixed a parsing bug on their side; it will sync correctly for all new leads going forward. They offered a historical export of past leads' `interest_model` values. Not yet decided whether to request it — open question for the business.
- Reason: act on the source system's actual behavior instead of our earlier, unverified guess (`meta_lead_id` as a distinct field) — avoids asking MGAgencia to send duplicate data and keeps the contract minimal.
- Impact: `MGAgenciaLeadRequest.cls` (removed `meta_lead_id` property), `MGAgenciaLeadValidator.cls` (removed its length check), `MGAgenciaLeadService.buildLead()` (`ID_Meta__c` now sourced from `external_lead_id`), test updates in `MGAgenciaLeadRestResourceTest.cls`, `docs/API_SPEC.md` §3 and `docs/FIELD_MAPPING.md` §13 updated. Still `condor-qas` only — `mi-org` deploy remains pending explicit user confirmation, same as the 2026-09-15 entry.
- Open question: whether to request MGAgencia's offered historical `interest_model` backfill export. Not resolved here.

## 2026-09-17 — `fb`/`ig` correction, guard-shift risk investigated, historical export cross-checked

- **`platform` values corrected**: a real MGAgencia historical lead export (`GIMC_MG_PU_260902_Leads.xls`, MGAgencia's own raw Meta Lead Ads data, 422 real rows) shows the actual values sent are `fb`/`ig` (Meta's own short codes) — never `facebook`/`instagram` as originally assumed on 2026-09-15. `MGAgenciaPlatformResolver`'s catalog corrected accordingly. `tiktok`/`web` remain unconfirmed against real traffic (no such rows seen in this sample).
- **Guard-shift automation risk investigated**: enabling `platform` → dynamic `LeadSource` (`Redes Sociales Empresa`/`Web`) re-exposes MGAgencia Leads to the org's `Asignacion_Lead_a_Vendedor` flow ("Asignación Lead a Vendedor según Calendario"), which the fixed `LeadSource = 'MGAgencia'` value has always avoided (that value simply never matches this flow's trigger criteria). A live test in `condor-qas` crashed with `INVALID_CROSS_REFERENCE_KEY` — but `condor-qas`'s guard-shift calendar data is known-sparse (see docs/FIELD_MAPPING.md §13.x on this sandbox limitation) and unrepresentative. Controlled, cleaned-up tests directly in `mi-org` with `LeadSource = 'Redes Sociales Empresa'` succeeded without error for **all 4 branches** (asunción, ciudad_del_este, coronel_oviedo, encarnación — each landed in queue `MQL`, same end state as today), confirming `mi-org` currently has active guard coverage everywhere. Caveat: this is a point-in-time snapshot (2026-09-17, mid-morning) — it does not prove 24/7 coverage (overnight/holiday gaps not tested) and depends on the vendor duty roster staying complete indefinitely, a dependency the current `MGAgencia`-fixed `LeadSource` never has.
- **Historical export cross-check** (same file): 422 real rows (1 excluded — MGAgencia's own leftover test/dummy row, values shifted one column). 417/422 matched an existing Lead in `mi-org` via `External_Lead_Id__c`; all 417 still have `VehicleOfInterestOfTheWeb__c` null (consistent with the 2026-09-16 `interest_model` finding). 5/422 have no matching Lead in `mi-org` despite being dated within the live integration window (2026-09-11 to 2026-09-16) — unexplained gap, not investigated further.
- Decision: **do not backfill** `VehicleOfInterestOfTheWeb__c` for the 417 historical Leads from this export. Explicit user call, no reason recorded.
- Impact: `MGAgenciaPlatformResolver.cls` and its test, `MGAgenciaLeadRequest.cls` doc comment, `MGAgenciaLeadRestResourceTest.cls`, `docs/API_SPEC.md` §3 updated. Still `condor-qas` only, PR #21 still not merged — blocked on the guard-shift branch-coverage decision above.

## 2026-09-22 — Production go-live: platform/Meta fields + logger fix deployed to `mi-org`

- MGAgencia's own client tested the full contract live against `condor-qas` (all 5 new fields + `test_drive_requested`) and got a real `500`. Root cause investigated and confirmed **unrelated to this feature**: `DatosInicialesProspecto` (a pre-existing org flow) creates a follow-up Task whenever `TestDriveRequested__c = true`, and the `Reglas_Meta`/`MQL` queues didn't support `Task` as an object type — same structural gap as ADMIN_FOLLOWUPS #5, different triggering flow. Live in production since `test_drive_requested` was added to the contract on 2026-09-03; never noticed because our own Apex test suite gives a false positive for this exact scenario (Salesforce test-context isolation hides the real automation dependency — same class of false confidence already seen with the calendar-guard flow). See `docs/ADMIN_FOLLOWUPS.md` #6 for full detail.
- Admin added `Task` to both `Reglas_Meta` and `MQL`'s supported object types, in both `condor-qas` and `mi-org`. Re-verified live in both orgs: `test_drive_requested: true` now returns `201`.
- MGAgencia re-ran their full test payload against `condor-qas` after the fix — `201`, all fields (including the corrected `fb` platform value) stored correctly.
- With that confirmed and the branch-coverage risk already investigated (2026-09-17 entry above), deployed PR #21 + PR #22 (merged together, one conflict in this file resolved additively — no semantic change) to `mi-org`: `RunSpecifiedTests` against the MGAgencia test classes, 37/37 passing. Live end-to-end verification in `mi-org` (controlled test Lead, immediately deleted): `201`, all 5 fields stored correctly, and the integration log record now has `Lead__c` populated (confirms PR #22's fix works in production too).
- Impact: this is a **zero-behavior-change deploy** for current real traffic — `platform` and the other 4 fields are optional/additive, and MGAgencia's actual production integration does not send them yet (still testing against Sandbox only). `LeadSource` keeps resolving to the fixed `MGAgencia` value for all real production leads until MGAgencia's production traffic starts sending `platform`. At that point, the guard-shift dependency documented on 2026-09-17 becomes live risk, not just a tested scenario — worth a fresh spot-check once that actually happens.
- Status: **LIVE in `mi-org`** as of 2026-09-22. No longer provisional/sandbox-only.
