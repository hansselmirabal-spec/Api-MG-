# Nebüla → Salesforce Leads API — Functional Specification v1.0 (draft for provider review)
...
**Purpose.** Nebüla collects advertising leads from marketing platforms and sends them to Grupo Cóndor's CRM (Salesforce) so they can be worked by our sales team, without manual data entry.

**Flow**

```
Nebüla  --HTTPS POST-->  Salesforce API
                              |
                              v
                        Field validation
                              |
...
| Item | Value |
|---|---|
| Endpoint path | `/services/apexrest/nebula/v1/leads` |
| HTTP method | `POST` |
...
**Token expiry and renewal.** Tokens expire according to the org's session policy. Nebüla should request a new token whenever a request fails with `401 Unauthorized`, rather than caching a token indefinitely or assuming a fixed lifetime.
...
One JSON object per request. No batch/array wrapping. Unknown properties sent by Nebüla are ignored (not rejected).
...
| `external_lead_id` | string | No *(see open questions)* | — | Free text | Unique identifier Nebüla may assign to a lead. Not required today; whether it becomes required and used as an idempotency key is open — see §10. | `"neb-2026-0000123"` |
...
| `integration_id` | string (UUID) | Reference identifier for this request. **Nebüla must store it** for support requests. It is not a Salesforce record identifier. |
...
Nebüla does not need to take corrective action on a duplicate response — it is flagged internally for reporting. Even so, Nebüla should avoid blindly retrying an already-successful request, since every accepted request currently creates a new lead record (see the idempotency note in §10, item 1).
...
|---|---|---|---|
| 1 | Can you send a unique `external_lead_id` per lead? | If confirmed, it becomes a **required** field and the idempotency key for safe retries (avoiding duplicate creation on resubmission). | Yes — Nebüla assigns and sends a stable, unique `external_lead_id` per lead. |
| 2 | How do you identify advertising campaigns? | Needed to attribute leads to campaigns for reporting. | Nebüla sends a `campaign_code` from a catalog agreed with Grupo Cóndor; we resolve it internally. |
| 3 | Do you need to send the vehicle model of interest? | Needed to route/report leads by product interest. | Nebüla sends an `interest_model` value from a catalog Grupo Cóndor provides. |
...
| Change communication | Any change to this contract (new/changed fields, status codes, or behavior) will be communicated to Nebüla in writing before deployment. Breaking changes are released under a new API version (§1); non-breaking changes are documented as an update to this specification. |
