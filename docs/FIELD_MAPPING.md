...
| `LeadSource` | `MGAgencia` (new picklist value) | Fixed value at insert, unless `platform` is sent (§13, PROVISIONAL 2026-09-15) — see `MGAgenciaPlatformResolver` | CONFIRMED — Decision 2 |
| Owner / queue | Ends up owned by queue `MQL` (amended 2026-09-08) | `MGAgenciaLeadService.insertLead` resolves the target queue from CMDT (`Queue_DeveloperName__c = 'Reglas_Meta'`) and sets `Lead.OwnerId` directly — not via the org's assignment-rule engine. The org's own automation then reassigns `Estatus__c = 'Nuevo'` Leads to queue `MQL`. Required a branch-label casing fix (`MGAgenciaBranchResolver` now writes `asunción`/`ciudad_del_este`/etc., lowercase_with_underscores, matching real Meta Leads and `Calendario_de_Asignaciones__c`) after 4 earlier failed attempts — see `docs/DECISIONS.md` Decision 1, fifth attempt, and `docs/ADMIN_FOLLOWUPS.md` #5 | CONFIRMED — Decision 1 (amended 2026-09-08) |
...
## 13. Admin/mkt request — dynamic LeadSource and Meta fields (2026-09-15, PROVISIONAL)

Source: "Cambios MG Agencia.docx", relayed by the admin/marketing team. Implemented on our side in `condor-qas` only, pending MGAgencia's confirmation that they can send the new request fields (see the heads-up note sent to MGAgencia). Not yet deployed to `mi-org`.

**LeadSource fields confirmed via `sf sobject describe -s Lead`**: `Redes Sociales Empresa` and `Web` are active in both `condor-qas` and `mi-org`. `Tiktok` (exact casing, capital T only) is active in `mi-org` but **not in `condor-qas`** — sandbox is stale on this value, same class of drift as Decision 7's branch-casing bug. A live sandbox test inserting `platform=tiktok` will fail in `condor-qas` for that reason alone; add the value there before relying on a full live test.

**4 new Lead fields confirmed via Tooling API `FieldDefinition`/`CustomField`** — all free text, no dependent picklist:
...
Searched every Flow/ApexTrigger/ApexClass in the org for references to `ID_Meta__c` / `Identificador_Lead_Capture__c`: **zero matches**. Nothing internal defines their semantics — they're populated by something outside our metadata (almost certainly a native Meta Lead Ads connector). Since no ground truth exists to reverse-engineer, the exact mapping of MGAgencia's raw data to these two fields is **our own contract design**, confirmed with the business owner in this conversation, not an invented Salesforce business rule.
