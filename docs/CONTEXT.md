...
MGAgencia collects Leads from advertising platforms and needs to insert them directly into our Salesforce org, removing manual processes.

Target flow:

Advertising platforms → MGAgencia → Salesforce REST API → Validation → Lead creation → Response to MGAgencia

## Reference document: Orbi Leads API Documentation v1.0

Provided by MGAgencia. Describes the API of another market. Technical reference only — nothing in it is assumed to apply here.
...
- Numeric catalog IDs (`campaign_id`, `concessionaire_id`, `years`, `brands`, `models`, `versions`) are provided by MGAgencia and must never be invented.
...
1. MGAgencia consumes our API and sends the Leads.
...
See CLAUDE.md: MGAgencia Lead identifier, campaign identification, Lead status query.
